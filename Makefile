...
test:
	...
	conda run -n test_tsvhold pip install -e . >/dev/null;
	conda run -n test_tsvhold python run_tests.py;
	conda run -n test_tsvhold pip install mypy >/dev/null;
	conda run -n test_tsvhold python -m mypy --strict .;
	conda run -n test_tsvhold python -m mypy --strict -p tsvhold;
